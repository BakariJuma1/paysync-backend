...
def send_payment_reminder_email(customer_email, customer_name, business_name, debt_details, reminder_type):
    ...
    if reminder_type == "before_due":
        ...
        subject = f"Payment Receipt - {debt_details.get('invoice_number', 'N/A')}"
    else:
        subject = f"Payment Notification: {business_name}"

    # Generate PDF
    pdf_buffer = generate_debt_pdf(debt_details)
    ...
    <body style="font-family: Arial, sans-serif; color: #333;">
        ...
        <h3 style="text-align:center;">{"Invoice Receipt" if reminder_type == "receipt" else "Invoice Notice"} - {debt_details.get('invoice_number', 'N/A')}</h3>
        <hr>
    ...
    try:
        response = resend.Emails.send({
            ...
            "attachments": [
                {
                    "filename": f"Invoice-{debt_details.get('invoice_number', 'N/A')}.pdf",
                    "content": pdf_base64,
                    "type": "application/pdf"
                }
            ]
        })
        logger.info(f"Email sent to {customer_email}. Type: {reminder_type}. Response: {response}")
        return True
    except Exception as e:
        logger.error(f"Failed to send email ({reminder_type}) to {customer_email}. Resend error: {e}")
        return False
